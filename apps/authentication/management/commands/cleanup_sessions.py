...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=30,
            help='Number of days after which to consider sessions expired (default: 30)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            if count > 0:
                self.stdout.write('Sessions to be deleted:')
                for session in expired_sessions[:10]:  # Show first 10
                    self.stdout.write(
                        f'  - {session.user.email} from {session.ip_address} '
